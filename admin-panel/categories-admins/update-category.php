    ...
    if (isset($_POST['submit'])) {
        ...
        if (empty($name)) {
          echo "<div class='alert alert-danger text-center' role='alert'> please type in something</div>";
             ...
             $queryinsert = $conn->prepare("update category set name = :name where id = $id");
             $queryinsert->execute([
                 ':name' => $name,
...
?>
       <div class="row">
        <div class="col">
          ...
          <form method="POST" action="update-category.php?up_id=<?php echo $result->id ?>">
                <!-- Email input -->
                <div class="form-outline mb-4 mt-4">
                  <input type="text" name="name" id="form2Example1" class="form-control" placeholder="<?php echo $result->name?>" />
